...
// Helper: Parse a hex string into an element in Zr.
...
bool verifyCredential(TIACParams &params,
    ...
    // --- Compute c' = Hash(g1, g2, h, com, com'', k, k'') ---
    ...
    // --- Check KoR: Verify that c_prime equals the c from the tuple ---
    bool kor_ok = (element_cmp(c_prime, c_elem) == 0);
    std::cout << "[VERIFY] KoR check: " << (kor_ok ? "PASSED" : "FAILED") << "\n";
    
    // --- Step 8: Pairing Check (Algorithm 18) ---
