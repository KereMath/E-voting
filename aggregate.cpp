...
// Helper: const element_t'yi non-const hale getirmek için – burada, element_t aslında element_s[1] olduğu için,
// dizinin ilk elemanının adresını kullanıyoruz.
static element_t toNonConst(const element_t in) {
...
AggregateSignature aggregateSign(
    ...
    element_set(aggSig.h, const_cast<element_t>(&partialSigsWithAdmins[0].second.h[0]));
    debugStream << "Aggregate h set from first partial signature.\n";
    
    // (2) s: Başlangıçta aggregate s, grup identity (1) olarak ayarlanır.
    ...
    for (size_t i = 0; i < partialSigsWithAdmins.size(); i++) {
        ...
        // Debug: Yazdır Lagrange katsayısını.
        char lambdaBuf[1024];
        ...
        element_pow_zn(s_m_exp, const_cast<element_t>(&partialSigsWithAdmins[i].second.s_m[0]), lambda);
