...
AggregateSignature aggregateSign(
    ...
    // partialSigsWithAdmins[0].second.h zaten element_t (yani element_s[1])
    // Dizinin ilk elemanının adresini alıyoruz:
    ...
    for (size_t i = 0; i < partialSigsWithAdmins.size(); i++) {
        ...
        // s_m değerinin ilk elemanının adresini alıp non-const pointer elde ediyoruz.
        std::string partStr = elementToStringG1(const_cast<element_s*>(&(partialSigsWithAdmins[i].second.s_m[0])));
        debugStream << "  Partial signature " << (i+1)
                    << " produced by Admin " << (adminID + 1)
                    << ": s_m = " << partStr << "\n";
        element_mul(aggSig.s, aggSig.s, const_cast<element_s*>(&(partialSigsWithAdmins[i].second.s_m[0])));
    }
