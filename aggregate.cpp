...
AggregateOutput aggregateSignatures(
    ...
    // all partials share same h => use partials[0].h
    // but partials[0].h is non-const, so no conversion error
    ...
    // 3) final => (h, s)
    // 4) optional check => e(h, alpha2 * beta2^DID) ?= e(s, g2)
    {
        // compute beta2^DID
        element_t beta2_pow, exp_did;
        element_init_G2(beta2_pow, params.pairing);
        element_init_Zr(exp_did, params.pairing);

        element_set_mpz(exp_did, in.DIDi);
        element_pow_zn(beta2_pow, in.beta2, exp_did);
        ...
        if (!ok) {
            std::cerr << "[WARN] aggregateSignatures: final check failed => 'Hata'\n";
        } else {
