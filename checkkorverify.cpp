 ...
 * (Alg.16'daki hashing ile tutarlılık için)
 */
static std::string elementToHexStr(const element_t elem, bool isG1) {
    // non-const kopyası
    element_t tmp;
...
/**
 * Helper: string -> element G1/G2
 */
static void stringToElementG1orG2(element_t result, const std::string &hexStr, pairing_t pairing, int gType) {
    // G1=1, G2=2
    switch (gType) {
        case 1:
    ...
    if (bytes.empty()) {
        throw std::runtime_error("stringToElementG1orG2: empty hex input");
    }
    if (element_from_bytes(result, bytes.data()) == 0) {
        throw std::runtime_error("stringToElementG1orG2: element_from_bytes failed");
    }
}
...
bool checkKoRVerify(
    ...
    const MasterVerificationKey &mvk,        // alpha2, beta2
...
)
{
    ...
    // proveRes içindeki k, c, s1, s2, s3
    element_t k_copy, c_copy, s1_copy, s2_copy, s3_copy;
    copyConstElement(k_copy,  proveRes.k,  params.pairing, 2); // k G2'de
    copyConstElement(c_copy,  proveRes.c,  params.pairing, 0); // c Zr
    copyConstElement(s1_copy, proveRes.s1, params.pairing, 0); // Zr
    ...
    copyConstElement(alpha2_copy, mvk.alpha2, params.pairing, 2); // G2
    copyConstElement(beta2_copy,  mvk.beta2,  params.pairing, 2); // G2
    ...
    // com_str -> G1
    element_t com_elem;
    stringToElementG1orG2(com_elem, com_str, params.pairing, 1);

    // 2) Hazırlık: (1-c) hesaplamak için
    //    c Zr'de -> 1 - c
    ...
    element_set1(one);
    element_sub(one_minus_c, one, c_copy); // (1 - c)

    // 3) Algoritma 17 Adım 1: k'' = g2^s1 * (alpha2)^(1-c) * k^c * (beta2)^(s2)
    ...
    // 4) Algoritma 17 Adım 2: com'' = g1^s3 * h^s2 * (com^c)
    ...
    // 5) Algoritma 17 Adım 3: c' = Hash(g1, g2, h, com, com'', k, k'')
    // Not: generateKoRProof içindeki hash sırası ile tutarlı olmalı.
    // Aynı sıralamayla: (g1, g2, h, com, com'', k, k'')
    std::ostringstream hashOSS;
    hashOSS << elementToHexStr(params.g1, true)  // g1 -> G1
            << elementToHexStr(params.g2, false) // g2 -> G2
            << elementToHexStr(h_copy, true)     // h -> G1
            << elementToHexStr(com_elem, true)   // com -> G1
            << elementToHexStr(com_prime_prime, true)
            << elementToHexStr(k_copy, false)    // k -> G2
            << elementToHexStr(k_prime_prime, false);
    ...
    if (mpz_set_str(c_prime_mpz, c_prime_hex.c_str(), 16) != 0) {
        ...
        // Bellek temizliklerini yapmadan return
        return false;
    }
