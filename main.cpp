...
std::cout << "========================\n\n";
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, voterCount),
        [&](const tbb::blocked_range<int>& r) {
            for (int i = r.begin(); i != r.end(); ++i) {
                ...
                try {
                    stringToElement(com_elem, preparedOutputs[i].com_str, params.pairing, 1); // debug.com yerine com_str kullanımı
    ...
    double totalPairing_ms = totalPairing_us.load() / 1000.0;
    ...
    std::cout << "Pairing Check      : " << totalPairing_ms << " ms\n";
    std::cout << "KoR Verification   : " << totalKorVer_ms << " ms\n";
