                         ...
                         element_t result)
{
    ...
    for (size_t i = 0; i < coeffs.size(); i++) {
        ...
        // power *= xVal  (bir sonraki terim için X^(k+1))
...
KeyGenOutput keygen(TIACParams &params, int t, int n) {
    ...
    // Polinom katsayıları: F_i(X), G_i(X) derece = t-1
    ...
    // Her EA_i'nin sabit terimi (x_i0, y_i0) -> EAKey.x_m, y_m
    for (int i=0; i<n; i++){
    ...
    // Master secret key (msk) = ( ∑ x_i0, ∑ y_i0 )
    ...
    // Master verification key (mvk) = (g2^sum(x_i0), g2^sum(y_i0), g1^sum(y_i0))
    ...
    // Her EA_i için imza payı ve doğrulama payı hesapla
