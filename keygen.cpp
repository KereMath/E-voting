...
KeyGenOutput keygen(TIACParams &params, int t, int ne) {
    ...
    // 2. Generate commitments Vxij, Vyij, Vyij_prime
    ...
    // 3. Verify shares using commitments
    ...
    // 4. Handle complaints and form subset Q
    std::vector<int> Q;
    for (int i = 0; i < ne; i++) {
        if (!complaints[i]) {
            Q.push_back(i);
    ...
    // 5. Use only authorities in Q to aggregate final sgk, mvk
    ...
    // 6. Generate EA keys for each authority in Q
    ...
    // 7. Release polynomial coefficients
    for (int i = 0; i < ne; i++) {
        for (int j = 0; j < t; j++) {
